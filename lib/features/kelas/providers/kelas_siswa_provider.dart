...
final kelasSiswaServiceProvider = Provider<KelasService>((ref) {
  return KelasService(ref.read(supabaseClientProvider));
});

final kelasSiswaListProvider =
    AsyncNotifierProvider<KelasSiswaListNotifier, List<KelasModel>>(
  KelasSiswaListNotifier.new,
);

class KelasSiswaListNotifier extends AsyncNotifier<List<KelasModel>> {
  late final KelasService _service = ref.read(kelasSiswaServiceProvider);
  ...
  Future<List<KelasModel>> build() async {
    ...
    if (uid == null) return <KelasModel>[];
    return _service.getAllMySiswa(uid);
  }

  Future<int?> getMy(int idKelas){
    final session = ref.watch(sessionProvider);
    final uid = session.userId;
  ...
  Future<void> refresh() async {
    ...
    final uid = session.userId;
    if (uid == null) {
      state = const AsyncData(<KelasModel>[]);
      return;
    }

    state = const AsyncLoading();
    state = await AsyncValue.guard(() => _service.getAllMySiswa(uid));
  }

  Future<void> add(KelasModel payload) async {
    final current = List<KelasModel>.from(state.value ?? <KelasModel>[]);
